...
class Inputs:
    ...
    def __post_init__(self):
        ...
        self.difficulty = self.compute_difficulty(variant = VARIANTS_ANGLE_SIN)

    def compute_difficulty(self, variant):
        q0 = np.array([ 1., 0., 0., 0.])
        t0 = np.array([ 0., 0., 0.])
        
        rotation_difficulty = quat_angle_error(label=self.q_est, pred=q0, variant=variant)[0, 0]
        translation_difficulty = np.linalg.norm(t0 - self.t_gt)
...
class MetricManager:
    ...
    def trans_err(inputs: Inputs) -> np.float64:
        # print(f"trans_err: {np.linalg.norm(inputs.t_est - inputs.t_gt)}")
        return np.linalg.norm(inputs.t_est - inputs.t_gt)

    @staticmethod
    @_metrics.register
    def rot_err(inputs: Inputs, variant: str = VARIANTS_ANGLE_SIN) -> np.float64:
        # print(f"rot_err: {quat_angle_error(label=inputs.q_est, pred=inputs.q_gt, variant=variant)[0, 0]}")
        return quat_angle_error(label=inputs.q_est, pred=inputs.q_gt, variant=variant)[0, 0]

    @staticmethod
    @_metrics.register
    def reproj_err(inputs: Inputs) -> float:
        # print(f"reproj_err: {reprojection_error(q_est=inputs.q_est, t_est=inputs.t_est, q_gt=inputs.q_gt, t_gt=inputs.t_gt, K=inputs.K, W=inputs.W, H=inputs.H)}")
    ...
    @staticmethod
    @_metrics.register
    def difficulty_rot(inputs: Inputs) -> float:
        return inputs.difficulty[0]

    @staticmethod
    @_metrics.register
    def difficulty_trans(inputs: Inputs) -> float:
        return inputs.difficulty[1]