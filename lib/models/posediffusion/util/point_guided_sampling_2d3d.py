...
def point_guided_sampling_2d3d(model_mean: torch.Tensor, t: int, disable_retry, matches_dict: Dict, uncropped_data: Dict, viz, pose_scale, pose_solver_conf, flip, multi_sample):
    ...
    # out['pgs2d3dloss'] = loss.item()
    # out['pose_solver_conf'] = pose_solver_conf

    # write_header = not os.path.isfile(f"./{os.environ['out_root']}/pgs2d3d_losses.csv")
    # with open(f"./{os.environ['out_root']}/pgs2d3d_losses.csv", 'a') as f:
...
def compute_point_distance(
    ...
    loss = pnp_solver_loss(i, relR, relT, uncropped_data, kp1, kp2, mask, pred_cameras, viz, gt_pose, rescale_factor)

    return loss, rescale_factor


def pnp_solver_loss(i, R, t, data, pts0, pts1, mask, camera, viz, gt_pose, rescale_factor):
    depth_0 = data['depth0'].squeeze(0)
    pts0 = pts0.to(dtype=torch.int32)
    pts1 = pts1.to(dtype=torch.int32)
    ...
    PI = torch.zeros((3,4), device=R.device)
    PI[0, 0] = 1
    PI[1, 1] = 1
    PI[2, 2] = 1
    ...
    transform[3,3] = 1
    transform[:3, :3] = R
    transform[:3, 3] = t #rescale_factor * t

    xyz0_h = (transform @ xyz0_h.T).T

    xyz0_projected_h = (PI @ xyz0_h.T).T

    xyz0_projected_p_h = (K1.to(dtype=torch.float32) @ xyz0_projected_h.T).T

    xyz0_projected_p = xyz0_projected_p_h[:, :2] / (torch.cat((xyz0_projected_p_h[:, 2].unsqueeze(1), xyz0_projected_p_h[:, 2].unsqueeze(1)), dim=1))

    loss = (torch.abs(pts1 - xyz0_projected_p)).mean()

    if viz is not None:
        cams_show = {"gt": gt_pose, "pred": camera}
