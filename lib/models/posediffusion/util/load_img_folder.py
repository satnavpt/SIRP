...
def preprocess_images(images, image_size: int = None, mode: str = "bilinear"):
    images_list = []
    bboxes_xyxy = []
    scales = []
    
    for image in images:
        image, bbox_xyxy, min_hw = _center_crop_square_tensor(image)
        ...
        images_list.append(imre.cpu().numpy())
...
def load_and_preprocess_images(folder_path  = None, image_size: int = None, image_paths = None, mode: str = "bilinear") -> torch.Tensor:
    ...
    for path in image_paths:
        ...
        if image_size is not None:
            ...
            print("no additional rescaling")
            imre = torch.from_numpy(image)[None]
