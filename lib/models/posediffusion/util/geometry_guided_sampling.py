...
from .camera_transform import pose_encoding_to_camera
...
def geometry_guided_sampling(model_mean: torch.Tensor, t: int, disable_retry, matches_dict: Dict, GGS_cfg: Dict, ):
    ...
    successful_GGS_count = 0

    # conduct GGS
    model_mean, c = GGS_optimize(model_mean, t, processed_matches, **GGS_cfg)

    successful_GGS_count += c

    # Optimize FL, R, and T separately
    model_mean, c = GGS_optimize(
    ...
    f.write(f"SUCCESSFUL GGS COUNT: {successful_GGS_count}\n")
    f.close()
    return model_mean
...
def GGS_optimize(
    ...
    **kwargs,
    ...
    return model_mean, successful_GGS_count
