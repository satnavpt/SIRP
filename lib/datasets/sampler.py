# From https://github.com/zju3dv/LoFTR/blob/261baf641cb9ada07dd9746e420ada7fe8a03152/src/datasets/sampler.py
import torch
...
class RandomConcatSampler(Sampler):
    ...
    def __iter__(self):
            ...
            if self.subset_replacement:
                ...
                if len_subset >= self.n_samples_per_subset:
                    ...
                    rand_tensor_replacement = torch.randint(
                        low, high, (self.n_samples_per_subset - len_subset,),
                        generator=self.generator, dtype=torch.int64)
        ...
        if self.repeat > 1:
            ...
            if self.shuffle:
                def _choice(x): return x[torch.randperm(len(x), generator=self.generator)]
